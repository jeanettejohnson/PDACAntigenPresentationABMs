...
def stardist = StarDist2D.builder(pathModel3)
        .threshold(0.001)              // Probability (detection) threshold
        .channels("DNA1")            // Select detection channel
        .normalizePercentiles(1, 99) // Percentile normalization
        .pixelSize(getCurrentServer().getPixelCalibration().getAveragedPixelSize() * 2) // grab resolution from image
    ...
    //    .tileSize(512)               // add tile size as per https://forum.image.sc/t/error-running-stardist-on-qupath-v0-3-0-rc2-using-opencv-converted-model/56216/5
        .build()
